...
import queue
import threading
from simulationSend import startSimulation

# Classe generata da pyuic5 (la tua Ui_MainWindow)
...
# Classe personalizzata per aggiungere logica
class SegatriceCNC(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.sim=threading.Thread(target=startSimulation, args=(self.coda,), daemon=True)
        self.sim.start()

        # Collegamento segnali ai pulsanti
        ...
        self.ui.dial.setEnabled(False)  # Dial disabilitato finché la macchina è spenta
        self.ui.dial.setValue(0)
        self.ui.lcd_velocita.display(0)

        self.timer = QtCore.QTimer(self)
        self.timer.timeout.connect(self.leggi_coda)
        self.timer.start(100)  # Controlla ogni 100 ms

    def toggle_macchina(self):
        """Accende o spegne la macchina"""
        if self.emergenza:
            return  # Non fare nulla se è attiva l'emergenza
        ...
        if self.macchina_accesa:
            ...
            self.lama_attiva = False  # Spegne anche la lama
    ...
    def toggle_lama(self):
        """Attiva o disattiva la lama"""
        if self.emergenza or not self.macchina_accesa:
            return  # Lama funziona solo se la macchina è accesa e non c'è emergenza
    ...
    def emergenza_attiva(self):
        ...
        # Per uscire dall'emergenza, premere di nuovo il pulsante (opzionale)
        self.emergenza = False

    def cambio_lama(self):
        """Simula il cambio della lama"""
        if self.emergenza or not self.macchina_accesa:
            return
        ...
        self.ui.dial.setEnabled(False)
        QtCore.QTimer.singleShot(2000, self.fine_cambio_lama)  # Simula un delay di 2 secondi

    def fine_cambio_lama(self):
        """Ripristina lo stato dopo il cambio lama"""
        if self.macchina_accesa:
    ...
    def aggiorna_velocita(self):
        """Aggiorna l'LCD in base al valore del dial"""
        if self.macchina_accesa:
            velocita = self.ui.dial.value()
            self.ui.lcd_velocita.display(velocita)

    def leggi_coda(self):
        try:
            while not self.coda.empty():
                data = self.coda.get_nowait()
                if isinstance(data, dict):
                    self.ui.lcd_velocita.display(data["speed"])
                self.coda.task_done()
        except queue.Empty:
            pass
