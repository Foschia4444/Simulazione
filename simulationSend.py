...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            # Invia lo stato della macchina ogni 1 secondo
            data = manager.band_saw.get_data()
            await websocket.send_json(data)
            await asyncio.sleep(0.5)  # Invia ogni 1/2 secondo
...
def startSimulation():
    manager.band_saw.redButton()  # Accende la macchina
    manager.band_saw.setMaxSpeed(80)
    manager.band_saw.startCutting()
    manager.band_saw.simulate(interval=1)
